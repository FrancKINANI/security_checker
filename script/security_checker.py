...
if not Path(COMPROMISED_PASSWORDS_FILE).is_file():
    logging.error("Le fichier de mots de passe compromis est introuvable : %s", COMPROMISED_PASSWORDS_FILE)
    exit(1)
...
def check_port(host, port):
    ...
    try:
        ...
        logging.error("Erreur lors de la vérification du port %s:%d : %s", host, port, e)
        return False
...
if __name__ == "__main__":
    ...
    logging.info("%d mots de passe faibles détectés. Rapport sauvegardé dans %s.", len(weak_passwords), args.output)
    ...
    if args.email:
        if args.sender_email and args.sender_password and args.recipient_email:
            try:
                with open(args.output, 'r', encoding='utf-8') as f:  # Ajout de encoding='utf-8'
            ...
            logging.error("Les informations d'email sont incomplètes. Le rapport ne sera pas envoyé.")
